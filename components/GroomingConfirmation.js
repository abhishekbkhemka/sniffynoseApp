...
export default class GroomingConfirmation extends Component {
    ...
    state = {breed:'select'}
      ...
      constructor(props){
        super(props)
          ...
          if(props.data.package){
              this.name = props.data.package.name + ' Package'
              this.packagePrice = props.data.package.price
              this.price = props.data.package.price
          }
          if(props.data.services && props.data.services.length > 0){
              ...
              this.name += ' + '+ services.join(',')
              this.price = this.packagePrice+' + '+ this.servicePrice
    ...
    confirmAppointment(){
        ...
        confirmationData['package'] = {name:this.props.data.package.name,price:this.props.data.package.price,notes:this.props.data.package.notes}
        confirmationData['date_time'] = this.props.data.date
        confirmationData['time_range'] = this.props.data.time
        confirmationData['price'] = this.packagePrice + this.servicePrice
        if(this.state.name){
            confirmationData['pet'] = {full_name:this.state.name,type:'dog',breed:this.state.breed,age:this.state.age}
        }else{
            confirmationData['pet'] = {full_name:'na',type:'dog',breed:this.state.breed,age:this.state.age}
        }

        confirmationData['notes'] = this.state.comments
...
<View style={{flex:1, justifyContent: 'flex-start', width:'96%' , margin: 'auto'}}>
    ...
    <View style={styles.confirmblock}>
          <View style={styles.appointdateblock}>
            <Text style={styles.dateblocknumber}>{this.props.data.date.getDate()}</Text>
            <Text style={styles.dateblocktext}>{MONTHS[this.props.data.date.getMonth()]}</Text>
            </View>
